...
def im_crop(frame_img, b, a,  width, height):
    box_size = max(width, height)
    roi = frame_img[max(int(b - box_size /2 ), 0) : min(int(b + box_size/2), cam_height), 
                    max(int(a - box_size/2 ), 0) : min(int(a + box_size/2 ), cam_width)]
    return roi
        ...
        for i in contours:
            ...
            if (x<len(contours)):
                if(cv2.contourArea(contours[x])>minContourArea):
                    ...
                    (a,b) = int((int(g)+int(w/2))), int((int(y) + int(h/2))) #centroid
                   # b, a = b,a
                    crop_img = im_crop(img_bw, b, a, w, h)
                    print("croped=",crop_img.shape)
                    svm_input = bad_good_process(crop_img)
                    svm_predict = svm_scorer.predict(svm_input)
                    print("svm_Predicted=",svm_predict)
