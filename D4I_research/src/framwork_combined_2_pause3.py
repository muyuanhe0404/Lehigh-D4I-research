...
def output_loc(cell_type):
    empty_array = np.empty((0, 4), int)
    arr = np.array(cell_type)
    locations = np.append(empty_array, arr, axis=0)
    return locations
## ^^^^^^^^^^^^^^****** ML prepartion ******^^^^^^^^^^^^^^^^ ##
...
try:
    ...
    while True:
        ...
        if (frame_num - INITIAL_PHASE) % PAUSE_PACE == 0:
            ...
            HCT_t = []
            WBC_t = []
            # type_location = np.empty((0, 4), int)
            for _ in contours:
                x += 1
                if (x < len(contours)):
            ...
            if(len(cropped_img)):          
                for index, pred in np.ndenumerate(pred.numpy()): 
                    if pred > 1:
                        ...
                        PC3_t.append([a_list[int(str(index[0]))],b_list[int(str(index[0]))],w_list[int(str(index[0]))],h_list[int(str(index[0]))]])       
                        g_counting += 1
                        t_counting += 1
                        ...
                        if pred == 1:
                            ...
                            i3 = cv2.circle(i3, coordinate[int(str(index[0]))], (50), (255), -1) 
                            wbc_counting += 1
                            WBC_t.append([a_list[int(str(index[0]))],b_list[int(str(index[0]))],w_list[int(str(index[0]))],h_list[int(str(index[0]))]])
                            if not args.inference:
                                debugging_info = cv2.rectangle(debugging_info, (g_list[int(str(index[0]))],y_list[int(str(index[0]))]), ((g_list[int(str(index[0]))]+w_list[int(str(index[0]))]),(y_list[int(str(index[0]))]+h_list[int(str(index[0]))])), (255,0,0), 3) #green means good PC3
                            
                        else:
                            print("HCT116")
                            cell_predicted = 'HCT'
                            hct_counting += 1   
                            HCT_t.append([a_list[int(str(index[0]))],b_list[int(str(index[0]))],w_list[int(str(index[0]))],h_list[int(str(index[0]))]])
                            if not args.inference:
            ...
            print('Enter the type of cell you wanna get the location of:')
            # cell_t = input()
            # if cell_t == 'PC3':
            #     loc = output_loc(PC3_t)
            #     print (loc)
            # elif cell_t == 'WBC':
            #     loc = output_loc(WBC_t)
            #     print (loc)
            # elif cell_t == 'HCT':
            #     loc = output_loc(HCT_t)
            #     print (loc)
        if not args.inference:
        ...
        print("PC3:", pc3_counting, "total good", g_counting)
        print("WBC:", wbc_counting, "total bad", b_counting)
        print("HCT:", hct_counting, "total bad", b_counting)
