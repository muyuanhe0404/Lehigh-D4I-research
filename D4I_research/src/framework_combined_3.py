...
def CNN_classify(cnn_classifier, single_cell_cropped, input_size = 112):
    with torch.no_grad():
        ...
        model_input = single_cell_cropped
        output =  cnn_classifier(model_input)
        score_output = F.softmax(output)
        score, pred = torch.max(score_output, 1)
        for x in range(len(pred.tolist())):
            if pred.tolist()[x] == 0:
                label_pred.append('PC3') 
            else: 
                label_pred.append('WBC') # ToDO: annotation library indicating the cell type
        print(label_pred)
        print(score.data)
...
def feature_save (details):
    ...
    with open("../data/features/"+filename, "a", encoding="utf-8") as f:
        f_csv = csv.writer(f)
...
all_data = []
labels = ['Type','Name', 'Location', 'Area', 'Diameter', 'Circularity','Mean','Standard_Deviation']
all_data = [labels]
filename = "features.csv"
                ...
                if(cv2.contourArea(contours[x]) > minContourArea):
                    ...
                    if svm_predict == 1:
                        if args.save:
                            ...
                            contour_area = cv2.contourArea(contours[x])/ (7.6 * 7.6)
                            ...
                            details.append([name,Location,contour_area,diameter,circularity,mean,std])
                        batch.append(trans(Image.fromarray(np.uint8(crop_img)).convert('RGB')).unsqueeze(0).to(device))
                        #batch.append((Image.fromarray(np.uint8(crop_img)).convert('RGB')))
        if len(batch) != 0:
            batch = torch.cat(batch, dim=0)
            #print(batch)
            pred,score = process_classifier(batch,cnn_classifier) 
            Type = pred
            details = np.insert(details,0,Type,axis = 1)
