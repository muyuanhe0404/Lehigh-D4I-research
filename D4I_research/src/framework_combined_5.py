...
try:
    ...
    while True:
        ...
        Prof_Liu = data.copy() # camera size
        for _ in contours:
            x += 1
            if (x < len(contours)):
                    ...
                    if svm_predict == 1:
                        ...
                        if pred == "PC3": # A propoer condition, here e.g. "PC3"
                            ...
                            if not args.inference:
                                Prof_Liu = cv2.rectangle(Prof_Liu, (g,y), ((g+w),(y+h)), (0,255,0), 3) #green means good PC3
                        else:
                            print("not PC3")

                            if not args.inference:
                                Prof_Liu = cv2.rectangle(Prof_Liu, (g,y), ((g+w),(y+h)), (0,0,255), 3) #red means good something else
                    else:
                        if args.save:
                            # bad_save(crop_img,b_counting)
                            bad_save(crop_img,x,frame_num)
                        if not args.inference:
                            Prof_Liu = cv2.rectangle(Prof_Liu, (g,y), ((g+w),(y+h)), (255,0,0), 3) #blue means bad anything
                        #b_counting = b_counting + 1
                    if not args.inference:
                        Prof_Liu = cv2.putText(Prof_Liu, (pred + ":" + ( "{:.2f}").format(score.item())), (g,y-30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))
                        Prof_Liu = cv2.putText(Prof_Liu, (( "{:3d}, {:3d}").format(a,b)), (g,y-60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))
                        
        frame_num  = frame_num + 1         
        if not args.inference:
            cv2.imshow('visualization',Prof_Liu)       
